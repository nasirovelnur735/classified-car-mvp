...
- "improve" — ТОЛЬКО улучшение качества исходной фотографии:
  • можно улучшать резкость, экспозицию, контраст, локальное освещение;
  • нельзя добавлять или удалять объекты;
  • нельзя менять автомобиль (цвет, форму, геометрию);
  • нельзя скрывать дефекты.

- "augment" — локальные реалистичные визуальные правки внешнего вида автомобиля:
  • можно визуально "помыть" авто, убрать поверхностные загрязнения;
  • можно добавить один реалистичный объект рядом с авто или на авто;
  • можно делать другие реалистичные локальные правки, не меняющие сущность автомобиля;
  • нельзя менять геометрию автомобиля;
  • нельзя менять фон, погоду, время суток и общую сцену;
  • нельзя скрывать реальные дефекты (царапины, вмятины, коррозию, повреждения).
...
- НЕ меняй автомобиль: цвет, форму и геометрию
- НЕ скрывай и не маскируй дефекты
- Разрешено:
  • улучшить резкость
  • улучшить экспозицию
  • улучшить контраст
  • улучшить локальное освещение
...
ЗАДАЧА:
Выполнить реалистичную локальную визуальную правку внешнего вида автомобиля
на исходном фото без изменения сущности автомобиля и сцены.

СТРОГИЕ ПРАВИЛА (ОБЯЗАТЕЛЬНЫ):
- Исходное изображение должно остаться ФОТОГРАФИЕЙ
- Разрешены локальные реалистичные правки:
  • визуально "помыть" автомобиль;
  • убрать поверхностные загрязнения;
  • добавить ОДИН реалистичный объект рядом с автомобилем или на автомобиль;
  • выполнить другие локальные реалистичные правки, не меняющие сущность автомобиля.
- Запрещено менять геометрию автомобиля
- Запрещено менять фон, погоду, время суток и общую сцену
- Запрещено скрывать реальные дефекты: царапины, вмятины, коррозию, повреждения
