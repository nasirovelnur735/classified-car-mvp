...
brand — ПРОИЗВОДИТЕЛЬ (компания-изготовитель): Toyota, BMW, Land Rover, Lada, Mercedes-Benz, Hyundai и т.д. Определяется по эмблеме или надписи производителя на решётке, крышке багажника, руле. Например: для Range Rover — brand = "Land Rover" (производитель), а не "Range Rover" (это модель);

model — МОДЕЛЬ или модельная линейка: Camry, X5, Range Rover, Niva Legend, E-Class и т.д. Определяется по шильдику с названием модели. Не путай: "Range Rover", "Defender", "Discovery" — это модели Land Rover; "X5", "3 Series" — модели BMW;
...
ВАЖНО: brand = производитель (Land Rover, Toyota, BMW), model = модельная линейка (Range Rover, Camry, X5).
Не путай: на многих авто самая заметная надпись — это модель; brand берётся с эмблемы производителя.
